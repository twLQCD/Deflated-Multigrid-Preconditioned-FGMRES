function [x,gdr,mvp,dots] = gmresproj(A,b,m,k,vk,hk,rtol,cyclim)
...
while ( (rnormiter > rtol) && (cycle <= cyclim) ) 
%
...
%
	...
	srvhere = srv(1:k+1,1);
		...
		x(:) = x(:) + vk(:,1:k)*d(1:k);
		wv = b(:) - A*x; %op(n,x(:,irhs)); %-may remove this and next line%
			r = wv;
...
%   fprintf(fid,'gmresproj(%d,%d) converged in %d cycles with a relative residual of %12.8g\n', ...
%           m,k,cycle-1,rn/rninit);
% else
%   fprintf(fid,'gmresproj(%d,%d) stoped after %d cycles without converging to the desired tolerence of %12.8g\n', ...
%           m,k,cycle-1,rn/rninit);
